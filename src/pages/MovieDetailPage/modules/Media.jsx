...
import { getMovieImg } from '../../../apis/movieDetailApi.js'
import { setMovieImg } from '../../../app/redux/slices/movieImgSlice.js'
...
const Media = () => {
  ...
  useEffect(() => {
    const getAPI = async () => {
      const { data } = await getMovieImg(movieId)
      dispatch(setMovieImg({ backdrops: data.backdrops, poster: data.posters }))
    }
    getAPI()
  }, [movieId, dispatch])
    ...
    <div className='pb-[37px] Media'>
      <div className='tab-pane pt-[30px] flex gap-x-5 items-center active'>
        ...
        <ul className='font-semibold mb-[20px] flex  flex-1 gap-x-10 text-[17.6px]'>
          <button type='button' className='after:opacity-100' onClick={(e) => toggleTap(e, 'Most Popular')}>
            Most Popular
            {!node && <span />}
          </button>
      ...
      <div className='tab-content relative h-[300px] rounded-xl'>
        ...
        <div tabid='Video' className='absolute inset-0 tab_item video '>
          <div className='flex items-center video rounded-tl-xl wrapper-video'>
            <SlideCard loadMore={listTrailer.length > 10 ? true : false}>
              {listTrailer &&
        ...
        <div tabid='Backdrop' className='absolute inset-0 tab_item'>
          <SlideCard loadMore={backdrops.length > 10 ? true : false} className='px-0 pb-0 rounded-xl gap-x-0'>
            {!_.isEmpty(backdrops) &&
        ...
        <div tabid='Poster' className='absolute inset-0 tab_item poster'>
          <SlideCard loadMore={poster.length > 10 ? true : false} className='px-0 pb-0 gap-x-0 rounded-xl'>
            {!_.isEmpty(poster) &&
